...
cost lap(int dim, const cost *restrict assign_cost,
  ...
  if (verbose) {
    printf("lapjv: COLUMN REDUCTION finished (%lf)\n", 
            static_cast<double>(duration_cast<microseconds>(system_clock::now() - start).count() / 1000));
  }
  start = system_clock::now();
  ...
  if (verbose) {
    printf("lapjv: REDUCTION TRANSFER finished (%lf)\n",
            static_cast<double>(duration_cast<microseconds>(system_clock::now() - start).count() / 1000));
  }
  ...
  for (int loopcnt = 0; loopcnt < 2; loopcnt++) {  // loop to be done twice.
    ...
    if (verbose) {
      printf("lapjv: AUGMENTING ROW REDUCTION %d / %d (%lf)\n",
              loopcnt + 1, 2,
              static_cast<double>(duration_cast<microseconds>(system_clock::now() - start).count() / 1000));
    }
  }  // for loopcnt
  ...
  for (idx f = 0; f < numfree; f++) {
    ...
    if (verbose) {
      printf("lapjv: AUGMENT SOLUTION row %d [%d / %d] (%lf)\n",
             freerow, f + 1, numfree,
             static_cast<double>(duration_cast<microseconds>(system_clock::now() - start).count() / 1000));
    }
  ...
  if (verbose) {
    printf("lapjv: AUGMENT SOLUTION finished (%lf)\n",
            static_cast<double>(duration_cast<microseconds>(system_clock::now() - start).count() / 1000));
  }
  ...
  if (verbose) {
    printf("lapjv: optimal cost calculated (%lf)\n",
            static_cast<double>(duration_cast<microseconds>(system_clock::now() - start).count() / 1000));
  }
