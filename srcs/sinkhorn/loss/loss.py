...
class DistCosineLoss(nn.Module):
    ...
    def __init__(self, modela=None, **kwargs):
...
class DistL2Loss(nn.Module):
    ...
    def forward(self, modelb):
        loss = 0
        num_params = 0
        for (name, p1), p2 in zip(self.modela.named_parameters(), modelb.parameters()):
            num_params += p1.numel()
            loss += torch.pow(p1 - p2, 2).sum()
            #loss += torch.pow(p1 - p2, 3).sum()
...
class MidLoss(nn.Module):
    ...
    def __init__(self, modela=None, criterion=None, **kwargs):
